...
void com_reset_buffers(void)
{
	inbufend = inbufplc = 0;
}


void com_send(long dat, long numbytes)
{
	long i;
...
void com_send_ptr(char *ptr, long len)
{
	long i = 0;
...
void com_send_string(char *string, int len)
{
	long i;
...
void comgetchar_timed(long count)
{
	short i;
...
void comwaitfor(char string[81]) // Wait for reply on serial (smartmodem control)
{
	short dat;
	long i;
...
void modem_stop_serial(void)
{
	
}

void modem_stop_modem(void)
{
...
void serial_send_my_sync_packet(void)
{
...
long
void serial_get_sync_packet(void)
{
	char syncbuf[80];
